...
class BodyGameRuntime(object):
    ...
    def run_network(self):
        ...
        while True:
            ...
            # 数据回传
            self.lock.acquire()
            self.result_pos.append("my_data")
            self.result_time.append(time.time())
            self.lock.release()
            time.sleep(5)
    ...
    def reconstruct(self):
        while True:
            self.lock.acquire()
            print(len(self.xyd_data))
            print(len(self.xyz_data))
            print(len(self.frame_time))
            print(len(self.color_frame))
            self.lock.release()
            time.sleep(1/20)
    ...
    def save_data(self):
        ...
        while True:
            ...
            io.savemat(path,{"xyddata":np.array(self.xyz_for_network),'time':np.array(self.frame_time)})
            self.lock.release()
            time.sleep(2)
