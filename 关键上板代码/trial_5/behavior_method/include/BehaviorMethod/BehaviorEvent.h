...
struct MYBPUModelWrapper {
  explicit MYBPUModelWrapper(BPU_MODEL_S bpu_model_) : bpu_model(bpu_model_) {}
  MYBPUModelWrapper() = default;
...
class BehaviorEvent {
 public:
  ...
  virtual int Init(const Json::Value &config,const std::string &cfg_path);

  std::vector<BaseDataPtr> Process(
      const std::vector<BaseDataPtr> &input,
      const xstream::InputParamPtr &param);

  virtual void RunSingleFrame(const std::vector<BaseDataPtr> &frame_input,
