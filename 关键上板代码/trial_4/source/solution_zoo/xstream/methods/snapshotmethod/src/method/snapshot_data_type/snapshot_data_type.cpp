...
SelectSnapShotInfoPtr SnapShotInfo::GetSnapShotInfo(
  ...
  auto ad_bbox = ImageUtils::AdjustSnapRect(frame->Width(), frame->Height(),
...
BaseDataVectorPtr SnapShotInfo::GenerateSnapshotInfo(
  ...
  for (auto &snap_info : snap_infos) {
    XStreamSnapshotInfoPtr xstream_snapshot_info(new XStreamSnapshotInfo());
    xstream_snapshot_info->value = CopySelectSnapShotInfo(snap_info);
    xstream_snapshot_info->value->type = type;
...
BaseDataPtr SnapShotInfo::GenerateWithoutSnapshot(const int32_t id) {
  ...
  cp_info->track_id = id;
  xstream_snapshot_info->value = cp_info;
  xstream_snapshot_info->type_ = "SnapshotInfo";
