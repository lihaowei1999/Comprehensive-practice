...
void DumpSkeleton(cv::Mat &frame,
  ...
  for (const auto &skeleton : skeletons->datas_) {
    auto p_kps = std::static_pointer_cast<XStreamData<Landmarks>>(skeleton);
...
void DumpLandmark(cv::Mat &frame,
  ...
  for (const auto &landmark : landmarks->datas_) {
    auto p_landmark =
      std::static_pointer_cast<XStreamData<Landmarks>>(landmark);
...
void DumpMask(cv::Mat &frame, const std::shared_ptr<BaseDataVector> &masks,
  ...
  for (size_t i = 0; i < body_box_size; ++i) {
    auto xstream_body_box =
        std::static_pointer_cast<XStreamData<BBox>>((body_boxes->datas_)[i]);
    auto body_box = xstream_body_box->value;
    int x1 = body_box.x1;
    int y1 = body_box.y1;
    int x2 = body_box.x2;
    int y2 = body_box.y2;
    ...
    auto xstream_mask =
        std::static_pointer_cast<XStreamData<Segmentation>>((masks->datas_)[i]);
    auto mask = xstream_mask->value;
    //    ofs << "(";
...
void DumpPlateWithInfo(cv::Mat &frame,
  ...
  for (uint32_t index = 0; index < plate_color->datas_.size(); ++index) {
    auto color_data = std::static_pointer_cast<XStreamData<Attribute<int>>>(
        plate_color->datas_[index]);
    auto row_data = std::static_pointer_cast<XStreamData<Attribute<int>>>(
