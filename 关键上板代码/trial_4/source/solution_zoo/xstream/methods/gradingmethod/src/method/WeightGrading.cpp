...
int WeightGrading::ProcessFrame(const std::vector<BaseDataPtr> &in,
  ...
  for (size_t i = 0; i < box_list->datas_.size(); i++) {
    std::shared_ptr<XStreamBBox> bbox;
    std::shared_ptr<XStreamPose3D> pose3d;
    std::shared_ptr<XStreamLandmarks> lmk;
    if (box_list->state_ == DataState::INVALID) {
      bbox = std::make_shared<XStreamBBox>();
    } else {
      bbox = std::static_pointer_cast<XStreamBBox>(box_list->datas_[i]);
    }
    if (pose_3d_list->state_ == DataState::INVALID) {
      pose3d = std::make_shared<XStreamPose3D>();
    } else {
      pose3d = std::static_pointer_cast<XStreamPose3D>(pose_3d_list->datas_[i]);
    }
    if (land_mark_list->state_ == DataState::INVALID) {
      lmk = std::make_shared<XStreamLandmarks>();
    } else {
      lmk =
          std::static_pointer_cast<XStreamLandmarks>(land_mark_list->datas_[i]);
    }

    std::shared_ptr<XStreamFloat> score(new XStreamFloat());
    score->type_ = "Number";

    if (in.size() == 4) {
      std::shared_ptr<XStreamQuality> quality;
      if (quality_list->state_ == DataState::INVALID) {
        quality = std::make_shared<XStreamQuality>();
      } else {
        quality =
            std::static_pointer_cast<XStreamQuality>(quality_list->datas_[i]);
      }
...
float WeightGrading::ProcessRect(const XStreamBBox &bbox) {
...
float WeightGrading::ProcessPose3d(const XStreamPose3D &pose3d) {
...
float WeightGrading::ProcessLmk(const XStreamLandmarks &lmk) {
...
float WeightGrading::ProcessQuality(const XStreamQuality &quality) {
