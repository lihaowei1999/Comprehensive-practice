...
int32_t ActPostPredictor::Init(std::shared_ptr<CNNMethodConfig> config) {
  ...
  auto thresholds_str = config->GetSTDStringValue("thresholds");
  LOGD << "default threshold_:" << threshold_;
  if (!thresholds_str.empty()) {
    LOGD << "class thresholds:" << thresholds_str;
  }
  ...
  if (thresholds_str.length() > 0) {
    ...
    while (delimiter != std::string::npos) {
      ...
      // remove current group from groups
      thresholds_str =
              thresholds_str.substr(delimiter + 1,
