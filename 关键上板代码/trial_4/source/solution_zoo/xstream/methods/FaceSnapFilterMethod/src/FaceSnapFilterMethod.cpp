...
bool FaceSnapFilterMethod::LmkVerification(\
  ...
  assert(lmk->value.values.size() == 5);
  if (filter_param_->lmk_filter_num > 0) {
    int filter_num = 0;
    for (size_t i = 0; i < 5; ++i) {
      if (lmk->value.values.at(i).score < filter_param_->lmk_thr[i]) {
        if (++ filter_num >= filter_param_->lmk_filter_num) {
          return false;
        }
      }
      if (static_cast<int>(lmk->value.values.at(i).x) >
          filter_param_->image_width ||
          static_cast<int>(lmk->value.values.at(i).x) < 0 ||
          static_cast<int>(lmk->value.values.at(i).y) < 0 ||
          static_cast<int>(lmk->value.values.at(i).y) >
          filter_param_->image_height)
        return false;
    }
  } else {
