...
int MattingPredictMethod::PrepareInputData(
  ...
  const int width = pyramid_image->Width();
  ...
  for (int32_t roi_idx = 0; roi_idx < box_num; roi_idx++) {
    ...
    // 2. kps
    auto kps = std::static_pointer_cast<XStreamData<hobot::vision::Landmarks>>(
    ...
    // 3. mask
    auto mask = std::static_pointer_cast<XStreamData<
    ...
    float roi_height = roi->value.Height();
    float roi_width = roi->value.Width();
    float roi_x1 = roi->value.x1 - 16 * roi_width / 224;
    float roi_y1 = roi->value.y1 - 16 * roi_height / 224;
    float roi_x2 = roi->value.x2 + 16 * roi_width/ 224;
    float roi_y2 = roi->value.y2 + 16 * roi_height / 224;
    BPU_ROI_S input_roi;
    ...
    int roi_origin_y = roi->value.y1;
    float ratio_x = 224.0 / roi->value.Width();
    float ratio_y = 224.0 / roi->value.Height();
    LOGD << "box " << roi_idx << " x1: " << roi_origin_x
         << ", y1: " << roi_origin_y;
    LOGD << "box " << roi_idx << " width: " << roi->value.Width()
         << ", height: " << roi->value.Height();

    auto kps_value = kps->value;
    ...
    for (size_t i = 0; i < kps_index.size(); i++) {
      ...
      cv::line(
          ske_kps,
          cv::Point(
              (kps_value.values[start_index].x - roi_origin_x) * ratio_x + 16,
              (kps_value.values[start_index].y - roi_origin_y) * ratio_y + 16),
          cv::Point(
              (kps_value.values[end_index].x - roi_origin_x) * ratio_x + 16,
              (kps_value.values[end_index].y - roi_origin_y) * ratio_y + 16),
          cv::Scalar::all(255), 1, CV_AA);
    }

    ske_kps.convertTo(ske_kps, CV_32FC1);

    int h_w = sqrt(mask->value.values.size());
    cv::Mat mask_224(h_w, h_w, CV_32FC1, mask->value.values.data());
