...
#include "VehicleTypePostProcessMethod/VehicleTypePostProcessMethod.h"
#include <memory>
#include <set>
#include <string>
...
void VehicleTypePostProcessMethod::ParseVehicleType(
  ...
  for (size_t roi_idx = 0, feature_idx = 0; roi_idx < valid_box.size();
    ...
    if (valid_box[roi_idx] != 1) {
      ...
      vehicle_type->value.value = max_index;
      vehicle_type->value.score = max_score;
