...
void MultitaskPostProcessMethod::GetKps2(
    BPU_MODEL_S bpu_model, std::vector<hobot::vision::Landmarks> &kpss,
    void *kps_label_output, void *kps_offset_output, int kps_label_level,
    int kps_offset_level, const std::vector<hobot::vision::BBox> &body_boxes) {
...
void MultitaskPostProcessMethod::GetLMKS1(BPU_MODEL_S bpu_model, int out_level,
  ...
  for (size_t box_id = 0; box_id < face_box_num; ++box_id) {
    ...
    for (int i = 0; i < lmk_points_number_; ++i) {
      ...
      landmark.values.push_back(hobot::vision::Point(x, y));
    }
...
void MultitaskPostProcessMethod::GetPose(BPU_MODEL_S bpu_model, int out_level,
  ...
  for (size_t box_id = 0; box_id < face_box_num; ++box_id) {
    ...
    pose.yaw = GetFloatByInt(mxnet_out_for_one_box[0], shift) * 90.0;
    LOGD << "pose.yaw: " << pose.yaw;
    pose.pitch = GetFloatByInt(mxnet_out_for_one_box[1], shift) * 90.0;
    LOGD << "pose.pitch: " << pose.pitch;
    pose.roll = GetFloatByInt(mxnet_out_for_one_box[2], shift) * 90.0;
