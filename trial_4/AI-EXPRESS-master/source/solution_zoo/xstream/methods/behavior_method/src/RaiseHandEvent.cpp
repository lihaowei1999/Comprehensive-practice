...
// 给isevent加一个
int RaiseHandEvent::IsEvent(hobot::vision::Landmarks kps) {
 ...
 */
  ...
  if (kps.values[9].score < skeleton_score_thres_ ||
    ...
    system("echo 'no raise' > /userdata/final/deploy/webservice/html/modules/data1.html");
    return false;
  }
  ...
  // right arm (Observed object left arm)
  {
    ...
    if (a < 0 && c < 0) {
      if (CalculateSlope(a, b)>= tan_threshold_) {
        ...
        system("echo 'left arm' > /userdata/final/deploy/webservice/html/modules/data1.html");
      }
    }
  }
  ...
  // left arm
  {
    ...
    if (a < 0 && c < 0) {
      if (CalculateSlope(a, b)>= tan_threshold_) {
        if(is_raise == true)
        {
          system("echo 'both arm' > /userdata/final/deploy/webservice/html/modules/data1.html");
        }else{
          system("echo 'right arm' > /userdata/final/deploy/webservice/html/modules/data1.html");
        }
        is_raise = true;
        LOGI << "right arm raise";
      }
  ...
  if (!is_raise) system("echo 'no raise' > /userdata/final/deploy/webservice/html/modules/data1.html");
  return is_raise;
}
