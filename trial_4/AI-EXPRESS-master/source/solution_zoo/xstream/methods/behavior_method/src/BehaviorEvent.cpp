...
void BehaviorEvent::RunSingleFrame(const std::vector<BaseDataPtr> &frame_input,
  ...
  for (size_t i = 0; i < boxes.size(); ++i) {
    ...
    int vote_res = IsEvent(kps->value); //01:raise  23:stand
    //3为坐下 2为站起
    if (vote_res==0 || vote_res==2) {
    ...
    auto vote_info_ptr = std::make_shared<XStreamData<
                           hobot::vision::Attribute<int32_t>>>();
    Vote(vote_info_ptr, track_id , vote_res);
    // output
    ...
    if (window_size < max_slide_window_size_) {
      ...
      if (voting_ratio > behavior_voting_threshold_) {
        vote_info_ptr->value.value = 1;
          if (vote_res>=2) {
            system("echo 'sit-down' > /userdata/final/deploy/webservice/html/modules/data2.html");
          }
      } else {
        vote_info_ptr->value.value = 0;//0 2 站起
           if (vote_res>=2) {
            system("echo 'stand' > /userdata/final/deploy/webservice/html/modules/data2.html");
          }
      }
