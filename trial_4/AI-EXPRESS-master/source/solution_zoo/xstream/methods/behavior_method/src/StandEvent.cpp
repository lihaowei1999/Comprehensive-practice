...
int StandEvent::IsEvent(hobot::vision::Landmarks kps) {
  //输出2或者3,3为坐下 2为站起
  if (kps.values[5].score < skeleton_score_thres_ ||
    ...
    //system("echo 'no body' >> /userdata/final/deploy/webservice/html/modules/data.html");
    return 2;
  }
  ...
  if (up_body_slope < slope_ || low_body_slope < slope_) {
    //system("echo 'bend' >> /userdata/final/deploy/webservice/html/modules/data.html");
    return 3;
  }
  ...
  // 膝胯、膝脚的角度
  {
    ...
    float angle_2 = CalculateAngle(kps.values[14],
                                   ...
                                   kps.values[16]);
    if (angle_1 < angle_ || angle_2 < angle_) {
      system("echo 'sit-down' >> /userdata/final/deploy/webservice/html/modules/data.html");
      return 3;
    }
  }
  system("echo 'stand' >> /userdata/final/deploy/webservice/html/modules/data.html");
  return 2;
}
