...
int SquatEvent::IsEvent(hobot::vision::Landmarks kps) {
  ...
  if (upbody_slope <= upper_body_) {
    system("echo 'on ground' >> /userdata/final/deploy/webservice/html/modules/data.html");
    return false;
  }
  ...
  if (b < epsilon || a/b > differ_) {
    system("echo 'Squat' >> /userdata/final/deploy/webservice/html/modules/data.html");
    LOGI<<"Squat detected success";
    return true;
  }
