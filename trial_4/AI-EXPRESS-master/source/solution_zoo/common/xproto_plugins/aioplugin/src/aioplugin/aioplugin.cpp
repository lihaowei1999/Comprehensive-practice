...
AioPlugin::AioPlugin(std::string config_path) {
...
int AioPlugin::ParseConfig(std::string config_file) {
...
int AioPlugin::DeInit() {
  LOGD << "AioPlugin::DeInit()";
  if (!micphone_device_)
    return -1;
...
int AioPlugin::Start() {
  ...
  if (micphone_device_) {
    micphone_thread_ =
      std::make_shared<std::thread>(&AioPlugin::MicphoneGetThread, this);
  }
  return 0;
}
...
int AioPlugin::MicphoneGetThread() {
  int ret = -1;
  ...
  while (!exit_) {
    ...
    if (ret > 0) {
     auto input = std::make_shared<AioMessage>(buffer, frames, audio_num_);
     audio_num_++;
     PushMsg(input);
    }
  }
