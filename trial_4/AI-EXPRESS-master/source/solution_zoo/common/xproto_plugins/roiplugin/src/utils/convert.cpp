...
using XRocImageFramePtr = xstream::XStreamData<ImageFramePtr>;
using XRocBBox = xstream::XStreamData<hobot::vision::BBox>;
using XRocPose3D = xstream::XStreamData<hobot::vision::Pose3D>;
using XRocLandmarks = xstream::XStreamData<hobot::vision::Landmarks>;
using XRocAge = xstream::XStreamData<hobot::vision::Age>;
using XRocAttribute = xstream::XStreamData<hobot::vision::Attribute<int32_t>>;
using XRocBaseDataVectorPtr = std::shared_ptr<xstream::BaseDataVector>;
using XRocSegmentation = xstream::XStreamData<hobot::vision::Segmentation>;
using XRocFeature = xstream::XStreamData<hobot::vision::Feature>;
using XRocAntiSpoofing = xstream::XStreamData<hobot::vision::AntiSpoofing>;
using XRocID = xstream::XStreamData<HorizonVisionTrackID>;
...
void CreateVisionBox(xstream::BaseDataPtr base, HorizonVisionBBox *vision_box) {
  ...
  BoxConversion<HorizonVisionBBox, hobot::vision::BBox>(vision_box,
                                                        xroc_box->value);
}
    ...
    if (with_debug_info) {
      ...
      if (nir_boxs) {
        for (size_t nir_i = 0; nir_i < nir_boxs->datas_.size(); ++nir_i) {
          auto nir_box = dynamic_cast<XRocBBox *>(
...
void ParseDisappredSmartData(HorizonVisionSmartData *targets,
  ...
  for (size_t i = 0; i < ids->datas_.size(); ++i) {
    auto id =
        dynamic_cast<XRocID *>(ids->datas_[i].get());
    HOBOT_CHECK(id);
...
void ParseHandSmartData(HorizonVisionSmartData *targets,
  ...
  for (size_t i = 0; i < hand_boxs->datas_.size(); ++i) {
    ...
    if (IsValidElement(gesture_votes, i)) {
      ...
      hand_data->hand_gesture = gesture_vote->value.value;
    }
