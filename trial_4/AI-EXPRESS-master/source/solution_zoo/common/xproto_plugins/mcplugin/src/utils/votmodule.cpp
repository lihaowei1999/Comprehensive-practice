  ...
  auto send_vot_func = [this](){
      ...
      while (!stop_) {
        ...
        if (frame_fps_ > 0 && dura + 5 < 1000 / frame_fps_) {
          // balance display realtime or smooth
          auto sleep_ms = (1000 / frame_fps_ - dura);  // /2;
          std::this_thread::sleep_for(std::chrono::milliseconds(sleep_ms));
          LOGD << "plot frame sleep_ms ms:" << sleep_ms;
        }
...
int VotModule::Init() {
  ...
  if (config_) {
    ...
    if (config_->HasKey("en_draw_head_box")) {
      vot_draw_ctrl_.draw_head_box = config_->GetBoolValue("en_draw_head_box");
    }
    if (config_->HasKey("en_draw_body_box")) {
      vot_draw_ctrl_.draw_body_box = config_->GetBoolValue("en_draw_body_box");
    }
    if (config_->HasKey("en_draw_face_lmk")) {
      vot_draw_ctrl_.draw_face_lmk = config_->GetBoolValue("en_draw_face_lmk");
    }
    if (config_->HasKey("en_draw_age_gender_dist")) {
      vot_draw_ctrl_.draw_age_gender_dist =
              config_->GetBoolValue("en_draw_age_gender_dist");
    }
    if (config_->HasKey("en_draw_body_kps")) {
      vot_draw_ctrl_.draw_body_kps = config_->GetBoolValue("en_draw_body_kps");
    }
  }
  ...
  if (en_encoder_) {
    ...
    if (ret) {
      if (ret == HB_ERR_VP_BUSY || (-1) *ret == HB_ERR_VP_BUSY) {
...
//                         1, CV_RGB(0, 0, 255), 1);
        ...
        if ("body_landmarks" == point.type_() && vot_draw_ctrl_.draw_body_kps) {
          static float body_kps_thr = 0.6;
          for (int lmk_idx = 0; lmk_idx < point.points__size(); lmk_idx++) {
            if (point.points_(lmk_idx).score_() >= body_kps_thr) {
              cv::circle(bgr,
                         cv::Point(point.points_(lmk_idx).x_(),
                                   point.points_(lmk_idx).y_()),
                         8, CV_RGB(163, 0, 20), -1);
...
//                          2, CV_RGB(255, 255, 255), 2);
          ...
          int lmk_ptr1_idx = 4;
          int lmk_ptr2_idx = 6;
          if (point.points_(lmk_ptr1_idx).score_() >= body_kps_thr &&
              point.points_(lmk_ptr2_idx).score_() >= body_kps_thr) {
            cv::line(bgr, cv::Point(point.points_(lmk_ptr1_idx).x_(),
                                    point.points_(lmk_ptr1_idx).y_()),
                     cv::Point(point.points_(lmk_ptr2_idx).x_(),
                               point.points_(lmk_ptr2_idx).y_()),
                     CV_RGB(255, 255, 255), 2, 2);
          }
...
//                        1, CV_RGB(255, 255, 255), 1);
            ...
            std::vector<int> start_idxs {1, 5, 9, 13, 17};
            for (const auto& start_idx : start_idxs) {
              cv::line(bgr, cv::Point(point.points_(start_idx).x_(),
                                      point.points_(start_idx).y_()),
                       cv::Point(point.points_(0).x_(), point.points_(0).y_()),
                       CV_RGB(255, 0, 0), 2, 2);
              for (int lmk_plot_idx = start_idx; lmk_plot_idx < start_idx + 3;
                   lmk_plot_idx++) {
                cv::line(bgr, cv::Point(point.points_(lmk_plot_idx).x_(),
                                        point.points_(lmk_plot_idx).y_()),
                         cv::Point(point.points_(lmk_plot_idx + 1).x_(),
                                   point.points_(lmk_plot_idx + 1).y_()),
                         CV_RGB(255, 0, 0), 2, 2);
              }
            }
          }
...
//                          d_color.at("face"), 2);
          ...
          if (vot_draw_ctrl_.draw_id) {
            int offset_factor = 1;
            if (s_data.track_id < 10) {
              offset_factor = 1;
            } else if (s_data.track_id < 100) {
              offset_factor = 2;
            } else {
              offset_factor = 3;
            }
            cv::putText(bgr, id_info.str(),
                        cv::Point2f(rect.x1 - 20 * offset_factor,
                                    rect.y1 + 20),
                        cv::HersheyFonts::FONT_HERSHEY_PLAIN,
                        1.5, CV_RGB(255, 255, 255), 2);
          }
