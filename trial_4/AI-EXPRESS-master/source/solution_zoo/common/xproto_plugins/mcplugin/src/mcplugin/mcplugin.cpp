...
using horizon::vision::xproto::smartplugin::target_key;
using horizon::vision::xproto::smartplugin::cmp_key;
...
int MCPlugin::Init() {
  ...
  RegisterMsg(TYPE_INFO_IMAGE_MESSAGE, std::bind(&MCPlugin::OnGetVioResult,
                                                 this, std::placeholders::_1));
  return XPluginAsync::Init();
}

std::shared_ptr<HorizonVisionImageFrame>
MCPlugin::ConstructVotImgData(const std::shared_ptr<VioMessage>& vio_) {
  HOBOT_CHECK(vio_);
  if (-1 == feed_vo_pym_layer_) {
    for (int layer = 0; layer < DOWN_SCALE_MAX; layer++) {
      if (feed_vo_dest_w_ == vio_->image_.front()->down_scale[layer].width &&
              feed_vo_dest_h_ ==
                      vio_->image_.front()->down_scale[layer].height) {
        feed_vo_pym_layer_ = layer;
        feed_vo_src_w_ = vio_->image_.front()->down_scale[0].width;
        feed_vo_src_h_ = vio_->image_.front()->down_scale[0].height;
  ...
  const auto& pym_info = vio_->image_.front()->down_scale[feed_vo_pym_layer_];
...
void MCPlugin::DumpSmart2Json(
        const std::shared_ptr<SmartMessage>& smart,
        const std::shared_ptr<std::string>& sp_smart_pb_) {
  auto custom_smart_msg = dynamic_cast<CustomSmartMessage *>(smart.get());
  HOBOT_CHECK(custom_smart_msg);
  uint64_t frame_id = custom_smart_msg->frame_id;
      ...
      for (int idx = 0; idx < target.points__size(); idx++) {
        ...
        if (point.type_() == "lmk_106pts") {
          for (int lmk_idx = 0; lmk_idx < point.points__size();
               lmk_idx++) {
            hobot::vision::Point pt(point.points_(lmk_idx).x_(),
                                    point.points_(lmk_idx).y_(),
                                    point.points_(lmk_idx).score_());
            smart_data.faces[track_id].lmk.values.emplace_back(pt);
          }
        }
        ...
        if ("body_landmarks" == point.type_()) {
          for (int lmk_idx = 0; lmk_idx < point.points__size(); lmk_idx++) {
            hobot::vision::Point pt(point.points_(lmk_idx).x_(),
                                    point.points_(lmk_idx).y_(),
                                    point.points_(lmk_idx).score_());
            smart_data.bodys[track_id].lmk.values.emplace_back(pt);
          }
        }
        ...
        if (point.type_() == "hand_landmarks") {
          for (int lmk_idx = 0; lmk_idx < point.points__size(); lmk_idx++) {
            hobot::vision::Point pt(point.points_(lmk_idx).x_(),
                                    point.points_(lmk_idx).y_(),
                                    point.points_(lmk_idx).score_());
            smart_data.hands[track_id].lmk.values.emplace_back(pt);
          }
        }
      }
...
//        }
  ...
  if (!smart_data.faces.empty()) {
    for (const auto& face : smart_data.faces) {
      ...
      faceInfo["faceQuality"] = face.second.box.score;
      faceInfo["faceFeatureNum"] = 0;
      faceInfo["facePictureNum"] = 0;

      Json::Value rect;
      rect["left"] = face.second.box.x1;
      rect["top"] = face.second.box.y1;
      rect["right"] = face.second.box.x2;
      rect["bottom"] = face.second.box.y2;
      ...
      faceInfo["faceKeyPointsNum"] = face.second.lmk.values.size();
      Json::Value lmks;
      for (const auto& val : face.second.lmk.values) {
        Json::Value lmk;
        lmk["x"] = val.x;
        lmk["y"] = val.y;
        lmk["z"] = -1;
        lmk["confidence"] = val.score;
        lmks.append(lmk);
      }
  ...
  if (!smart_data.hands.empty()) {
    for (const auto& hand : smart_data.hands) {
      ...
      Json::Value rect;
      rect["left"] = hand.second.box.x1;
      rect["top"] = hand.second.box.y1;
      rect["right"] = hand.second.box.x2;
      rect["bottom"] = hand.second.box.y2;
      handInfo["rectangle"] = rect;

      handInfo["keyPointsNum"] = hand.second.lmk.values.size();
      Json::Value lmks;
      for (const auto& val : hand.second.lmk.values) {
        Json::Value lmk;
        lmk["x"] = val.x;
        lmk["y"] = val.y;
        lmk["z"] = -1;
  ...
  if (!smart_data.bodys.empty()) {
    for (const auto& body : smart_data.bodys) {
      ...
      bodyInfo["faceID"] = -1;
      bodyInfo["quality"] = body.second.box.score;
      bodyInfo["bodyDistance"] = -1;
      bodyInfo["attributeNum"] = 0;

      Json::Value rect;
      rect["left"] = body.second.box.x1;
      rect["top"] = body.second.box.y1;
      rect["right"] = body.second.box.x2;
      rect["bottom"] = body.second.box.y2;
      bodyInfo["rectangle"] = rect;

      bodyInfo["keyPointsNum"] = body.second.lmk.values.size();
      Json::Value lmks;
      for (const auto& val : body.second.lmk.values) {
        Json::Value lmk;
        lmk["x"] = val.x;
        lmk["y"] = val.y;
        lmk["z"] = -1;
...
MCPlugin::ConstructVotSmartData(const std::shared_ptr<SmartMessage>& smart_) {
  HOBOT_CHECK(smart_);
...
std::shared_ptr<std::string>
MCPlugin::DumpSmartData(const std::shared_ptr<SmartMessage>& smart,
                        const std::shared_ptr<std::string>& sp_smart_pb_) {
  auto custom_smart_msg = dynamic_cast<CustomSmartMessage *>(smart.get());
  ...
  ss_dump_smart << custom_smart_msg->frame_id;
  ...
  for (const auto &output : smart_result->datas_) {
      ...
      for (size_t i = 0; i < face_boxes->datas_.size(); ++i) {
        ...
        if (face_box->value.id != -1) {
          ...
          if (prefix == "hand") {
            ...
            ss_hand << face_box->value.x1 * x_ratio
                    << " " << face_box->value.y1 * y_ratio
                    << " " << face_box->value.x2 * x_ratio
                    << " " << face_box->value.y2 * y_ratio
                    << " " << face_box->value.score
                    << " " << prefix
                    << " " << face_box->value.id;
            if (ss_dump_hand.find(face_box->value.id) != ss_dump_hand.end()) {
              ss_dump_hand[face_box->value.id].ss_hand = std::move(ss_hand);
            } else {
              gesture_dump_t gesture_output;
              gesture_output.ss_hand = std::move(ss_hand);
              ss_dump_hand[face_box->value.id] = std::move(gesture_output);
            }
          } else {
            ss_dump_smart << " " << face_box->value.x1 * x_ratio
                          << " " << face_box->value.y1 * y_ratio
                          << " " << face_box->value.x2 * x_ratio
                          << " " << face_box->value.y2 * y_ratio
                          << " " << face_box->value.score
      ...
      for (size_t i = 0; i < hand_lmks->datas_.size(); ++i) {
        ...
        if (smart_target.find(hand_key) == smart_target.end()) {
          ...
          for (size_t i = 0; i < lmk->value.values.size(); ++i) {
            ss_hand_lmk << " " << std::round(lmk->value.values[i].x * x_ratio)
                        << " " << std::round(lmk->value.values[i].y * y_ratio)
                        << " " << lmk->value.values[i].score;
          }
          if (ss_dump_hand.find(hand_box_list[i]->value.id) !=
                  ss_dump_hand.end()) {
            ss_dump_hand[hand_box_list[i]->value.id].ss_hand <<
                                                             ss_hand_lmk.str();
          }
        }
      }
    ...
    if (output->name_ == "gesture") {
      ...
      for (size_t i = 0; i < gesture_raws->datas_.size(); ++i) {
        auto gesture_raw = std::static_pointer_cast<
        ...
        if (smart_target.find(hand_key) == smart_target.end()) {
          ...
          if (ss_dump_hand.find(hand_box_list[i]->value.id) !=
                  ss_dump_hand.end()) {
            ss_dump_hand[hand_box_list[i]->value.id].gest_raw =
                    gesture_raw->value.value;
            ss_dump_hand[hand_box_list[i]->value.id].gest_score =
                    gesture_raw->value.score;
          }
        }
      }
    }
    ...
    if (output->name_ == "gesture_model_out") {
      ...
      if (gesture_model_outs->datas_.size() != hand_box_list.size()) {
        LOGE << "gesture_model_outs size: " << gesture_model_outs->datas_.size()
             << ", body_box size: " << hand_box_list.size();
        continue;
      }
      ...
      for (size_t i = 0; i < gesture_model_outs->datas_.size(); ++i) {
        ...
        if (smart_target.find(hand_key) == smart_target.end()) {
          ...
          for (const auto& val : gesture_model_out->value.values) {
            model_out_values.push_back(val);
          }
          if (ss_dump_hand.find(hand_box_list[i]->value.id) !=
                  ss_dump_hand.end()) {
            ss_dump_hand[hand_box_list[i]->value.id].model_out_values =
    ...
    if (output->name_ == "gesture_vote") {
      ...
      for (size_t i = 0; i < gesture_votes->datas_.size(); ++i) {
        ...
        if (smart_target.find(hand_key) == smart_target.end()) {
          ...
          if (ss_dump_hand.find(hand_box_list[i]->value.id) !=
                  ss_dump_hand.end()) {
            ss_dump_hand[hand_box_list[i]->value.id].gest_vote =
                    gesture_vote->value.value;
          }

          if (ss_dump_hand.find(hand_box_list[i]->value.id) !=
                  ss_dump_hand.end()) {
            ss_dump_hand[hand_box_list[i]->value.id].gest_final = gesture_ret;
          }
        }
      }
    }
...
int MCPlugin::Start() {
  ...
  if (config_ && config_->HasKey("enable_auto_start") &&
    ...
    if (enable_vot_) {
      ...
      if (horizon::vision::VotModule::Instance(vot_cfg)->Init() != 0 ||
              horizon::vision::VotModule::Instance()->Start() != 0) {
...
int MCPlugin::Stop() {
  ...
  if (auto_start_ && enable_vot_) {
    ...
    horizon::vision::VotModule::Instance()->DeInit();
  }
...
int MCPlugin::OnGetVioResult(const XProtoMessagePtr& msg) {
  ...
  if (enable_dump_img_) {
    std::string pic_name {save_dump_img_path_ + "/pym0" +
                "_" + std::to_string(valid_frame->image_.front()->frame_id) +
                "_" + std::to_string(valid_frame->image_.front()->Width()) +
                "_" + std::to_string(valid_frame->image_.front()->Height()) +
                ".nv12"};
    LOGD << "pic_name:" << pic_name;
    std::ofstream ofs(pic_name);
    if (ofs.is_open()) {
      ofs.write(reinterpret_cast<char*>(valid_frame->image_.front()->Data()),
                valid_frame->image_.front()->DataSize());
      ofs.write(reinterpret_cast<char*>(valid_frame->image_.front()->DataUV()),
                valid_frame->image_.front()->DataUVSize());
    }
  }
