 ...
 * All rights reserved.
 ***************************************************************************/
#ifndef APP_INCLUDE_PLUGIN_WAREPLUGIN_WAREMESSAGE_H_
#define APP_INCLUDE_PLUGIN_WAREPLUGIN_WAREMESSAGE_H_
#include <memory>
#include <string>
#include <vector>
...
using horizon::vision::xproto::smartplugin_multiplebox::RecogResult_t;
...
typedef xstream::XStreamData<hobot::vision::Feature> XStream_Feature;
typedef std::shared_ptr<XStream_Feature> XStream_Feature_ptr;
...
class FeatureWrapper {
 public:
  ...
  explicit FeatureWrapper(hobot::vision::Feature &feature,
                          std::string img_url);
  ~FeatureWrapper();
...
class RecordWrapper {
 public:
   ...
   * */
  void Serialize(const int& page_num,
                 const int& page_id,
                 x3ware::WareMessage *ware_msg);
  void Print(void);
...
class WarePluginRecogMessage : public XProtoMessage {
 public:
  WarePluginRecogMessage() = delete;
  explicit WarePluginRecogMessage(std::shared_ptr<RecogResult_t> data) {
    type_ = TYPE_RECOG_MESSAGE;
    message_data_ = data;
  }
  ~WarePluginRecogMessage() = default;
  std::shared_ptr<RecogResult_t> GetMessageData() { return message_data_; }
  std::string Serialize() {return "don't serialize";}

 private:
  std::shared_ptr<RecogResult_t> message_data_;
};
...
#endif  // APP_INCLUDE_PLUGIN_WAREPLUGIN_WAREMESSAGE_H_
