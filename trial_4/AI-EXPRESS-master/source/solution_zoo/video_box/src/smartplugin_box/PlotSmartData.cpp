...
int PlotSmartData::PlotData(const smart_vo_cfg_t vo_confg,
  ...
  auto plot_box =
      [](cv::Mat &bgr_, const HorizonVisionBBox &box,
        ...
        cv::rectangle(bgr_,
                      cv::Point(box.x1 * x_scale0 + x0,
                                box.y1 * y_scale0 + y0),
                      cv::Point(box.x2 * x_scale0 + x0,
                                box.y2 * y_scale0 + y0),
                      color, thickness);
      };
...
#else
        // recog id
        std::unordered_map<uint64_t, std::shared_ptr<RecogResult_t>>::iterator
  ...
  bgr_to_nv12(bgr.ptr<uint8_t>(), (uint8_t *)video_data->buffer, width_tmp,  //NOLINT
              height_tmp);
  delete[] bgr_buf;
