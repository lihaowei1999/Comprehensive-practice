...
void ParseFaceSmartData(
    ...
    const horizon::vision::FaceQuality *face_quality,
  ...
  for (size_t i = 0; i < face_boxs->datas_.size(); ++i) {
    ...
    horizon::vision::utilbox::CreateVisionBox(face_boxs->datas_[i],
                                              &face_data->face_rect);
    if (IsValidElement(head_box, i)) {
      horizon::vision::utilbox::CreateVisionBox(head_box->datas_[i],
                                                &face_data->head_rect);
    }
    if (IsValidElement(pose, i)) {
      horizon::vision::utilbox::CreateVisionPose(pose->datas_[i],
                                                 &face_data->pose3d);
    }
    face_data->landmarks = nullptr;
    if (IsValidElement(lmk, i)) {
      HorizonVisionAllocLandmarks(&face_data->landmarks);
      horizon::vision::utilbox::CreateVisionLmk(lmk->datas_[i],
                                                face_data->landmarks);
    }
    if (IsValidElement(age, i)) {
      horizon::vision::utilbox::CreateVisionAge(age->datas_[i],
                                                &face_data->age);
    }
    if (IsValidElement(gender, i)) {
      horizon::vision::utilbox::CreateVisionAttribute(gender->datas_[i],
                                                      &face_data->gender);
    }
    if (IsValidElement(face_quality->blur, i)) {
      horizon::vision::utilbox::CreateVisionAttribute(
    ...
    if (with_debug_info) {
      ...
      if (IsValidElement(rgb_norm_rois, i)) {
        horizon::vision::utilbox::CreateVisionBox(rgb_norm_rois->datas_[i],
      ...
      if (nir_boxs) {
        for (size_t nir_i = 0; nir_i < nir_boxs->datas_.size(); ++nir_i) {
          auto nir_box = dynamic_cast<horizon::vision::utilbox::XRocBBox *>(
      ...
      if (IsValidElement(nir_lmk, nir_idx)) {
        ...
        horizon::vision::utilbox::CreateVisionLmk(nir_lmk->datas_[nir_idx],
      ...
      if (match && IsValidElement(nir_norm_rois, nir_idx)) {
        horizon::vision::utilbox::CreateVisionBox(
            nir_norm_rois->datas_[nir_idx], &extra_info->nir_norm_box_);
      }
      // nir box
      if (match && IsValidElement(nir_boxs, nir_idx)) {
        horizon::vision::utilbox::CreateVisionBox(nir_boxs->datas_[nir_idx],
                                                  &extra_info->nir_box_);
      }
...
void ParseDisappredSmartData(HorizonVisionSmartData *targets,
  ...
  for (size_t i = 0; i < ids->datas_.size(); ++i) {
    auto id =
        dynamic_cast<horizon::vision::utilbox::XRocID *>(ids->datas_[i].get());
    HOBOT_CHECK(id);
...
HorizonVisionSmartFrame *Convertor::ConvertOutputToSmartFrame(
  ...
  horizon::vision::FaceQuality face_quality;
