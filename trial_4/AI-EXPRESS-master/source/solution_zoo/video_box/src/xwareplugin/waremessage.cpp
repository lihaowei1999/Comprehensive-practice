...
FeatureWrapper::FeatureWrapper(hobot::vision::Feature &feature,
                               std::string img_url) {
  xwhfeature_.feature_attr_ = feature_attr_ = ((1 << 6) | 1);
  strncpy(xwhfeature_.img_uri_, img_url.c_str(), img_url.size() + 1);
  LOGD << "feature.values.size:" << feature.values.size();
  if (feature.values.size() <= 0) return;
  for (auto &fea : feature.values) feature_f_.emplace_back(fea);
  xwhfeature_.size_ = feature_f_.size();
  xwhfeature_.feature_ = &feature_f_[0];
  Print();
}
