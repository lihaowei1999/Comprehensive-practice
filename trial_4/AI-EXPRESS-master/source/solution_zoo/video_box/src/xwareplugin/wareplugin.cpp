...
#include "xwareplugin/wareplugin.h"

XPLUGIN_REGISTER_MSG_TYPE(XPLUGIN_FEATURE_MESSAGE)
XPLUGIN_REGISTER_MSG_TYPE(XPLUGIN_RECOG_MESSAGE)
using horizon::vision::xproto::smartplugin_multiplebox::SmartFeatureMessage;
namespace horizon {
namespace vision {
namespace xproto {
...
int WarePlugin::Init() {
  ...
  RegisterMsg(TYPE_FEATURE_MESSAGE,
              std::bind(&WarePlugin::OnGetFeatureResult, this,
                        std::placeholders::_1));
  ...
  if (feature_msg->GetMessageType() == "Pic_Face") {
    if (features->feature_infos_.size() > 0) {
      if (features->feature_infos_[0].float_arrays_.size() > 0) {
        std::shared_ptr<FeatureWrapper> feature_w =
            std::make_shared<FeatureWrapper>(features->feature_infos_[0].
            float_arrays_[0], "faceid");
    ...
    for (auto &feat_info : features->feature_infos_) {
      for (auto &feature : feat_info.float_arrays_) {
        ...
        std::shared_ptr<WareData> result_ptr = std::make_shared<WareData>();
        auto ret = db_table_->Search(recog_dis_thr_, recog_sim_thr_,
            feature_w, result_ptr);
        if (!ret) {
          for (auto &result : result_ptr->search_result_) {
        ...
        if (!ret && result_ptr->search_match_ == true) {
          auto recog_info = std::make_shared<RecogResult_t>();
          ...
          auto recog_msg = std::make_shared<WarePluginRecogMessage>(recog_info);
          PushMsg(recog_msg);
