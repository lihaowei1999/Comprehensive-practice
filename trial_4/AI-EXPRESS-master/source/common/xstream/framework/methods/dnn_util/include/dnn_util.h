...
inline int FilterRoi(hobot::vision::BBox *src, hobot::vision::BBox *dst,
...
inline int NormalizeRoi(hobot::vision::BBox *src, hobot::vision::BBox *dst,
                        NormParams norm_params, uint32_t total_w,
                        uint32_t total_h, FilterMethod filter_method) {
  *dst = *src;
  ...
  float w_new = box_w;
  float h_new = box_h;
  NormMethod norm_method = norm_params.norm_method;
