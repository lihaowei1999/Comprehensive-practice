 ...
 * @File: DnnPredictMethod.cpp
 * @Brief: definition of the DnnPredictMethod
 * @Author: zhe.sun
 * @Email: zhe.sun@horizon.ai
 * @Date: 2020-11-23 11:15:05
 * @Last Modified by: zhe.sun
 * @Last Modified time: 2029-11-23 16:17:08
 */
...
int32_t DnnPredictMethod::Init(const std::string &cfg_path) {
  ...
  dnn_is_sync_ = config_["dnn_is_sync"].isBool() ?
                 config_["dnn_is_sync"].asBool() : dnn_is_sync_;
  dnn_run_with_roi_ = config_["dnn_run_with_roi"].isBool() ?
                      config_["dnn_run_with_roi"].asBool() : dnn_run_with_roi_;
  dnn_model_group_ = config_["dnn_model_group"].isBool() ?
                     config_["dnn_model_group"].asBool() : dnn_model_group_;
  dnn_model_group_id_ = config_["dnn_model_group_id"].isInt() ?
                        config_["dnn_model_group_id"].asInt() : 0;
  int core_id = config_["core_id"].isInt() ?
                config_["core_id"].asInt() : 0;  // 默认为0
  ...
  if (dnn_model_group_) {
    ...
    HOBOT_CHECK(ret == 0) << "set model group failed: "
...
void DnnPredictMethod::Finalize() {
  dnn_model_ = nullptr;
}

int DnnPredictMethod::AllocInputTensor(
  ...
  for (int i = 0; i < dnn_model_->bpu_model.input_num; i++) {
    ...
    switch (tensor.data_type) {
      case BPU_TYPE_IMG_NV12_SEPARATE: {
        ...
        ret = HB_SYS_bpuMemAlloc("in_data1", uv_length, true, &tensor.data_ext);
        if (ret != 0) {
          LOGE << "bpu alloc mem failed: " << HB_BPU_getErrorName(ret);
          // release alloced mem
      ...
      case BPU_TYPE_IMG_Y: {
        ...
        ret = HB_SYS_bpuMemAlloc("in_data0", image_length, true, &tensor.data);
...
int DnnPredictMethod::AllocOutputTensor(
...
int DnnPredictMethod::AllocOutputTensor(
    std::vector<BPU_TENSOR_S> &output_tensors, int num) {
  output_tensors.resize(dnn_model_->bpu_model.output_num);
  for (int i = 0; i < dnn_model_->bpu_model.output_num; i++) {
    BPU_TENSOR_S &tensor = output_tensors[i];
    BPU_MODEL_NODE_S &node = dnn_model_->bpu_model.outputs[i];
    ...
    LOGD << "output_size: " << output_size;
    int ret =
        HB_SYS_bpuMemAlloc("out_data0", output_size * num, true, &tensor.data);
    if (ret != 0) {
      LOGE << "bpu alloc mem failed: " << HB_BPU_getErrorName(ret);
      // release alloced mem
      FreeTensor(output_tensors);
...
void DnnPredictMethod::FreeTensor(std::vector<BPU_TENSOR_S> &tensors) {
    ...
    if (dnn_run_with_roi_) {
      ...
      for (size_t i = 0; i < input_tensor.size(); ++i) {
        ...
        ret = HB_BPU_runModel(&dnn_model_->bpu_model,
                              input_tensor[i].data(), input_tensor[i].size(),
                              output_tensor[i].data(), output_tensor[i].size(),
                              &dnn_ctrl_, dnn_is_sync_, &task_handle[i]);
        if (ret != 0) {
          LOGE << "HB_BPU_runModel failed: " << HB_BPU_getErrorName(ret);
