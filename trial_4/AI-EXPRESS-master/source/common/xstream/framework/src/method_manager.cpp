 ...
 * @brief     MethodManager interface of xstream framework
 * @author    shuhuan.sun
...
void MethodManager::Init(const Config &config,
                         const XStreamSharedConfig &shared_config,
  ...
  unique_name_ = config_[kUniqueName].asString();
  ...
  if (method_instance_count == 1) {
    if (config_.isMember(kMethodCfgString) && 0 == methods_[0]->
            InitFromJsonString(config_[kMethodCfgString].toStyledString())) {
      state = MethodManagerContextState::INITIALIZED;
    } else if (0 == methods_[0]->Init(config_[kMethodCfgPath].asString())) {
      state = MethodManagerContextState::INITIALIZED;
    } else {
  ...
  if (methodinfo.is_thread_safe_) {
    ...
    if (state != MethodManagerContextState::INITIALIZED) {
      for (size_t i = 0; i < methods_.size(); ++i) {
        if (config_.isMember(kMethodCfgString) && 0 == methods_[i]->
              InitFromJsonString(config_[kMethodCfgString].toStyledString())) {
          LOGD << "methods " << i << " initial by method_config!";
        } else if (0 != methods_[i]->Init(config_[kMethodCfgPath].asString())) {
    ...
    if (methodinfo.is_src_ctx_dept) {
      ...
      for (size_t i = 0; i < methods_.size(); i++) {
        contexts_[i % thread_count]->method_list_.insert({0, methods_[i]});
      }
    }
  }

  thread_pool_.reset(new XThreadPool(unique_name_, threads_, prepares, params),
                     [](XThreadPool *thp) {
                       thp->Stop();
                       delete thp;
                     });
...
void MethodManager::InitMethod(void *ptr) {
  ...
  for (auto method : ctx->method_list_) {
    if (config_.isMember(kMethodCfgString) && 0 == method.second->
            InitFromJsonString(config_[kMethodCfgString].toStyledString())) {
      LOGD << "method Init by method_config_string";
    } else if (0 != method.second->Init(config_[kMethodCfgPath].asString())) {
      exit(1);
...
uint32_t MethodManager::GenMethodKey(
    const std::vector<std::vector<BaseDataPtr>> &inputs,
    const std::vector<InputParamPtr> &params, size_t source_id) {
...
void MethodManager::Process(const std::vector<std::vector<BaseDataPtr>> &inputs,
  ...
  xstream::MethodPtr method = c->method_list_[method_key];
