 ...
 * @brief     provides config for xstream
 * @author    chuanyi.yang
...
int XStreamConfig::LoadFile(const std::string& file_path) {
  ...
  if (cfg_jv_.isMember("version")) {  // parse config
    ...
    Config workflows = cfg_jv_["workflow"];
    HOBOT_CHECK(workflows.size() > 0) << "invalid workflows";
  }
  // 不允许"method_config_file", "method_config"同时使用
  Config workflows = cfg_jv_["workflow"];
  HOBOT_CHECK(workflows.size() > 0) << "invalid workflows";
  for (uint32_t i = 0; i < workflows.size(); i++) {
...
SchedulerConfig::SchedulerConfig(XStreamConfigPtr config) {
  config_ = config;
  ...
  for (int i = 0; i < node_size; ++i) {
    ...
    auto node_name = one_node[kUniqueName].asString();
...
bool ParseConfig::IsWorkflowObject(const Json::Value& config) {
  if (config.isObject() && config.isMember("type") &&
      config["type"].asString() == "workflow" && config.isMember("workflow") &&
      config["workflow"].isArray() && config.isMember("inputs") &&
      config["inputs"].isArray() && config.isMember("outputs") &&
      config["outputs"].isArray() && config.isMember("name") &&
...
bool ParseConfig::IsTemplateRefObject(const Json::Value& config) {
  if (config.isObject() && config.isMember("type") &&
      ...
      config.isMember("template_name") && config["template_name"].isString() &&
      config.isMember("parameters") && config["parameters"].isObject()) {
    LOGI << "template_ref:" << config["template_ref"].asString() << std::endl;
    return true;
  }
  return false;
}
...
int ParseConfig::ParseWorkflows(Json::Value& workflows,
  ...
  switch (workflows.type()) {
    case Json::objectValue:  // json
    {
      ...
      for (auto iter = mem.begin(); iter != mem.end(); iter++) {
        if ((ret = ParseWorkflows(workflows[*iter], templates,
                                  repeat_parsing)) != 0)
          return ret;
      }
      break;
    ...
    case Json::arrayValue: {  // array
      for (uint i = 0; i < workflows.size(); i++) {
        if ((ret = ParseWorkflows(workflows[i], templates, repeat_parsing)) !=
            0)
          return ret;
      }
      break;
...
void ParseConfig::GetNodes(Json::Value& root, Json::Value& output) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    for (uint j = 0; j < root_workflow["workflow"].size(); j++) {
      ...
      if (sub_type == "workflow") {
        ...
        GetSubNodes(sub_workflow, nodes, sub_name, sub_workflow["inputs"],
                    sub_workflow["outputs"], sub_workflow["__arg_inputs__"],
                    sub_workflow["__arg_outputs__"]);
...
int ParseConfig::GetWorkflows(Json::Value& config, Json::Value& out) {
  ...
  for (int level = 0; repeat_parsing && level < 10; level++) {
    ...
    if ((ret = ParseWorkflows(config["workflows"], templates_map,
                              repeat_parsing)) != 0) {
      return ret;
    }
  }
