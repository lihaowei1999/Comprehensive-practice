...
std::vector<std::vector<BaseDataPtr>> BBoxFilter::DoProcess(
  ...
  // do thread_priority test.
  if (has_thread_params_) {
  #if defined(HR_LINUX) && defined(HR_POSIX)
    sched_param sch;
    int old_policy;
    pthread_getschedparam(pthread_self(), &old_policy, &sch);
    if (thr_policy_ == "SCHED_FIFO") {
      HOBOT_CHECK(old_policy == SCHED_FIFO &&
    ...
    } else if (thr_policy_ == "SCHED_RR") {
      HOBOT_CHECK(old_policy == SCHED_RR &&
      sch.sched_priority == thr_priority_)
      << "Thread policy should be " << thr_policy_
      << ", priority=" << thr_priority_ << ", but "
      << "actually policy:" << old_policy
