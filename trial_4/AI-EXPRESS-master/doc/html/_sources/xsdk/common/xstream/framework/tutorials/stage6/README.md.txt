...
```json
// common/xstream/framework/tutorials/stage6/config/filter.json
 1    {
 2      "inputs": [
 3        "head_box"
 4      ],
...
```C++
// xstream/framework/tutorials/stage6/include/method/bbox.h
...
```C++
// xstream/framework/tutorial/stage6/include/method/bbox_filter_a.h
...
```C++
// xstream/framework/tutorial/stage6/include/method/bbox_filter_a.cc
std::vector<std::vector<BaseDataPtr>> BBoxFilterA::DoProcess(
    const std::vector<std::vector<BaseDataPtr>> &input,
    const std::vector<InputParamPtr> &param) {
  RUN_FPS_PROFILER("BBoxFilterA")
...
在使用BBoxFilterA和BBoxFilterB类之前，要注册这两个类到xstream的MethodFactory中。对应xstream/framework/tutorial/stage6/config/filter.json中的method_type字段。

```C++
// xstream/framework/tutorials/stage6/include/method/method_factory.cc
...
```C++
// xstream/framework/tutorial/stage6/src/main.cc
...
进入xstream/framework/目录下，执行如下命令，运行stage6_main

```bash
cd xstream/framework/tutorial/stage6
