...
### 定义BBoxFilter Method
现在需要实现BBoxFilter Method的核心功能。XStream中定义了Method基础类，我们需要在此基础上实现自定义的BBoxFilter，并根据需要实现`DoProcess`这个核心处理函数，以及扩充成员变量，如置信度阈值`score_threshold_`。
...
### 注册BBoxFilter Method到MethodFactory
