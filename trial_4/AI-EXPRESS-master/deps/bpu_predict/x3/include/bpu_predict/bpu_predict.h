 ...
 */
int BPU_loadModel(const char *model_file_name,
                  BPUHandle *handle,
...
/*
 * \brief get error description string of the last encouter problem. if no
 * problem has been happened, just return "OK".
 */
 ...
 * \return:
 *  name_list, pointer of char* array, the memory was allocated by lib, and do
 * not write or free them name_list_cnt, number of name in name_list.
 */
int BPU_getModelNameList(BPUHandle handle,
...
struct BPUModelInfo {
  ...
  int *is_big_endian;         // is big endian or not
 ...
 * suppose num of data is 2, and input1's shape is (1, 224, 224, 3), input2's
 * shape is (2,6) then: num is 2 ndim_array is [0, 4, 6], the 0 is input1's
 * shape start, 4 is input1's shape end and input2's shape start shape_array is
 * [1, 224, 224, 3, 2, 6], you can use ndim as shape index: input1's shape is
 * shape_array[ndim_array[0]] ~ shape_array[ndim_array[1]] input2's shape is
 * shape_array[ndim_array[1]] ~ shape_array[ndim_array[2]]
 *
 ...
 */
int BPU_getModelInputInfo(BPUHandle handle,
 ...
 */
int BPU_getModelOutputInfo(BPUHandle handle,
                           const char *model_name,
                           BPUModelInfo *info);
...
/*
 * get pointer of BPU Buffer data. the return pointer is void*, and can be cast
 * to other data type
 */
 ...
 *  when the model is running, this function will cause current thread to wait
 * for running done. when the model has already done, this function will return
 * immediately.
 */
 ...
 */
const char *BPU_getModelLastError(BPUHandle handle,
...
/*
 * \brief run model from ddr, which had been format into 8P8C before call this
 * function.
 */
int BPU_runModelFromDDR(BPUHandle handle,
                        const char *model_name,
                        BPU_Buffer_Handle input[],
                        int nInput,
                        BPU_Buffer_Handle output[],
                        int nOutput,
                        BPUModelHandle *model_handle,
 ...
 */
int BPU_runModelFromDDRWithConvertLayout(BPUHandle handle,
                                         const char *model_name,
                                         BPU_Buffer_Handle input[],
                                         int nInput,
                                         BPU_Buffer_Handle output[],
 ...
 *  this function will use roi to select one of input image, crop and resize to
 * proper size of model's input-data and then run model from the resized
 * input-data ATTENTION: bbox may be change in this function! output is the
 * noramlized bbox
 */
 ...
 */
int BPU_setGroupProportion(int group_id, int proportion);

/*
 * \brief delete a group id
 */
int BPU_deleteGroup(int group_id);

enum BPULayoutType { LAYOUT_1HW1 = 0, LAYOUT_111C, LAYOUT_1111, LAYOUT_NHWC };
/*
 * \brief convert layout.
 */
int BPU_convertLayout(BPUHandle handle,
                      void *to_data,
                      const void *from_data,
                      const char *model_name,
                      BPULayoutType layout_type,
 ...
 */
int BPU_setModelPrior(BPUHandle handle);
