...
typedef enum {
  HBRT_VERSION_RUNTIME = 0,
  HBRT_VERSION_COMPILER,
  HBRT_VERSION_CPU_OPERATOR,
  HBRT_VERSION_NUMBER_OF_VERSIONS,
} hbrt_version_type_t;
 ...
 */
#define HBRT_MAX_CORE_NUMBER (2U)
#define HBRT_MAX_HBM_NUMBER (32767U)
#define HBRT_MAX_CORE_MASK (1U << HBRT_MAX_CORE_NUMBER)
#define HBRT_MAX_OUTPUT_NUMBER (32U)
#define HBRT_MAX_RI_NUMBER (256U)

// @formatter:off
// clang-format off
// NOTE: msg must be a single string without concat due to the restriction of the preprocessor
#define MAX_CORE_NUMBER                                                                                              \
  HBRT_PRAGMA_WARN(                                                                                                  \
      "MAX_CORE_NUMBER has been superseded by HBRT_MAX_CORE_NUMBER since 3.6.1, and will be removed in a future release")                                                                                               \
  HBRT_MAX_CORE_NUMBER

#define MAX_HBM_NUMBER                                                                                               \
  HBRT_PRAGMA_WARN(                                                                                                  \
      "MAX_HBM_NUMBER has been superseded by HBRT_MAX_HBM_NUMBER since 3.6.1, and will be removed in a future release. There is no hbm number limit since 3.9.0. HBM file can be loaded as long as the memory is enough.")                                                                                               \
  HBRT_MAX_HBM_NUMBER

#define MAX_CORE_MASK                                                                                                \
  HBRT_PRAGMA_WARN(                                                                                                  \
      "MAX_CORE_MASK has been superseded by HBRT_MAX_CORE_MASK since 3.6.1, and will be removed in a future release")                                                                                               \
  HBRT_MAX_CORE_MASK

#define MAX_OUTPUT_NUMBER                                                                                            \
  HBRT_PRAGMA_WARN(                                                                                                  \
      "MAX_OUTPUT_NUMBER has been superseded by HBRT_MAX_OUTPUT_NUMBER since 3.6.1, and will be removed in a future release")                                                                                               \
  HBRT_MAX_OUTPUT_NUMBER

#define MAX_RI_NUMBER                                                                                                \
  HBRT_PRAGMA_WARN(                                                                                                  \
      "MAX_RI_NUMBER has been superseded by HBRT_MAX_RI_NUMBER since 3.6.1, and will be removed in a future release")                                                                                               \
  HBRT_MAX_RI_NUMBER
// clang-format on
// @formatter:on
...
/**
 * @brief Release all memory allocated inside hbrt.
 ...
 */
HBDK_PUBLIC extern hbrt_error_t hbrtGetModelMarch(hbrt_march_t *march, hbrt_model_handle_t model_handle);
...
typedef struct {
  ...
  /* Note: pyramid batch input use 'resizer_img_height' to calculate the start address of each data frame,
   * For model with pyramid batch input, resizer_img_height must be set properly */
  uint32_t resizer_img_height;       /// base image height, useful for pyramid/resizer input
  uint32_t resizer_img_width;        /// base image width, useful for pyramid/resizer input
...
typedef struct {
  ...
  } separate_output_regions[HBRT_MAX_OUTPUT_NUMBER];
...
#define HBRT_MAX_BATCH_SIZE 4096U

// clang-format off
// @formatter:off
#define MAX_BATCH_SIZE                                                                                          \
  HBRT_PRAGMA_WARN(                                                                                             \
      "MAX_BATCH_SIZE has been superseded by HBRT_MAX_BATCH_SIZE since 3.6.1, and will be removed in a future release")                                                                                         \
  HBRT_MAX_BATCH_SIZE
// @formatter:on
// clang-format on
...
HBDK_PUBLIC extern hbrt_error_t hbrtBilinearRoiResizeImage(
    ...
    bpu_addr_t output_bpu_addr, uint32_t ri_id, hbrt_march_t march, uint32_t interrupt_num);
...
HBDK_PUBLIC extern hbrt_error_t hbrtBilinearRoiResizeFeatureMap(
    ...
    hbrt_march_t march, uint32_t interrupt_num, bool keep_hw_ration);
 ...
 * @param [in] keep_hw_ration if true, it will keep the height to width ration of the original ROI. The valid result
 * will be at the left-top corner.
 * @param [in] y_pad_value pad value of Y channel when ROI exceeds image boundary
 * @param [in] u_pad_value pad value of U channel when ROI exceeds image boundary
 * @param [in] v_pad_value pad value of V channel when ROI exceeds image boundary
 * @return ::hbrtSuccess, ::hbrtErrorMemoryAllocationFail, ::hbrtErrorInvalidRoi,
 * ::hbrtErrorIllegalMARCH.
 * @note
 *       1. the input image should be in YUV420NV12 format and the output image will be in YUV444 format.
 *       2. this api will generate BPU instructions and function call(s) for platform specified by march.
 */
HBDK_PUBLIC extern hbrt_error_t hbrtBilinearRoiResizeImageExt(
    void **funccall_buffer, uint32_t *generated_funccall_num, bpu_addr_t image_y_addr, bpu_addr_t image_uv_addr,
    uint32_t image_height, uint32_t image_width, uint32_t image_h_stride, bool uv_enable, hbrt_pad_mode_t pad_mode,
    const hbrt_roi_t *p_rois, uint32_t roi_num, uint32_t dest_w, uint32_t dest_h, hbrt_layout_type_t output_layout,
    bpu_addr_t output_bpu_addr, uint32_t ri_id, hbrt_march_t march, uint32_t interrupt_num, bool keep_ratio,
    uint32_t y_pad_value, uint32_t u_pad_value, uint32_t v_pad_value);

/**
 * Insert context switch funccalls at the head and tail of funccall buffer
 * @param [in,out] p_funccall_buffer       in: original funccall(s) generated by RiStart/RiContinue,
 *                                         out: funccalls after inserting context switch funccalls
 * @param [out] generated_funccall_number  number of funccall(s) in funccall buffer
 * @param [in]  ri_id                      the id to assign to this running instance
 * @return ::hbrtSuccess, ::hbrtErrorRiIsInUse, ::hbrtErrorInvalidModelHandle, ::hbrtErrorIllegalRIConfig,
 * ::hbrtErrorInvalidResizerParam, ::hbrtErrorInvalidSegmentIndex, ::hbrtErrorMemoryAllocationFail,
 * ::hbrtErrorIllegalMARCH, ::hbrtErrorFunccallSlotNotEnough
 */
HBDK_PUBLIC extern hbrt_error_t hbrtRiEnablePreemption(void **p_funccall_buffer, uint32_t *generated_funccall_num,
                                                       uint32_t ri_id);
/**
 ...
 * @param [in] ri_id id of the ri.
 * @return ::hbrtSuccess, ::hbrtErrorIllegalRIID, ::hbrtErrorRiIsNotInUse
 * @note This function is supposed to be called after the caller has received interrupt from BPU.
 */
...
typedef hbrt_data_work_t DATA_WORK HBRT_DEPRECATED_NAME(hbrt_data_work_t, DATA_WORK, 3.7.1);
 ...
 * ::hbrtErrorBPUCPUMemAllocFail, ::hbrtErrorMemoryOverflow, ::hbrtErrorInvalidDataWork
 */
HBDK_PUBLIC extern hbrt_error_t hbrtRiGetOutputData(void **data, uint32_t ri_id, uint32_t output_index,
                                                    hbrt_data_work_t work);
...
/**
 * Check if two versions are compatible. Only versions with same major number are compatible
 * @param v1 version1
 * @param v2 version2
...
static inline hbrt_error_t hbrtIsCompatibleHeader(void) {
  ...
  snprintf(header_version.version, sizeof(header_version.version) - 1U, "%u.%u.%u", header_version.major,
           header_version.minor, header_version.patch);
  header_version.type = HBRT_VERSION_RUNTIME;
  hbrt_error_t compatibility_err = hbrtIsCompatibleVersion(header_version, ver);
  if (hbrtSuccess != compatibility_err) {
    fprintf(stderr,
            "The program is compiled with header files from hbrt version %s. It's not compatible"
            " with the linked hbrt library %s\n",
            header_version.version, ver.version);
  }
  return compatibility_err;
}
